...
std::string DeriveKitToml(const KitProbeData& data) {
    ...
    // /v2 added [gate] (noise floor per raw byte). /v1 readers ignore
    // [gate] and behave as before; /v2 readers honour it.
    os << "schema       = \"shadps4-legacy-instrument/v2\"\n";
    ...
    // /v2 noise gate. For drum kits only — walk each drum byte and
    // emit a gate threshold = baseline_max + small margin. PS4 RB drum
    // modules behind a legacy adapter (PDP / MadCatz / Riffmaster pads)
    // have non-zero idle bias on some channels; the gate ensures that
    // idle bias doesn't fire a phantom press.
    if (data.device_type == ProbeDeviceType::Drum || data.device_type == ProbeDeviceType::ProDrum) {
        constexpr int kSafetyMargin = 4;
        struct DrumByte {
            const char* step;
            int byte;
        };
        const DrumByte drum_bytes[] = {
            {"red_pad", velByte("red_pad")},         {"blue_pad", velByte("blue_pad")},
            {"yellow_pad", velByte("yellow_pad")},   {"green_pad", velByte("green_pad")},
            {"kick_pedal", velByte("kick_pedal")},   {"yellow_cymbal", velByte("yellow_cymbal")},
            {"blue_cymbal", velByte("blue_cymbal")}, {"green_cymbal", velByte("green_cymbal")},
        };
        std::ostringstream gate_buf;
        bool any_gate = false;
        std::array<bool, 256> seen_byte{};
        for (const auto& d : drum_bytes) {
            if (d.byte < 0 || d.byte >= (int)baseline_max.size())
                continue;
            if (seen_byte[d.byte])
                continue;
            seen_byte[d.byte] = true;
            const int base = baseline_max[d.byte];
            if (base <= 0)
                continue;
            int gate = base + kSafetyMargin;
            if (gate > 255)
                gate = 255;
            gate_buf << '"' << d.byte << "\" = " << gate << "  # " << d.step << '\n';
            any_gate = true;
        }
        if (any_gate) {
            os << "\n[gate]\n" << gate_buf.str();
